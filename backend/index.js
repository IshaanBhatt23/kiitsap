...
const purchaseOrderFuse = new Fuse(purchaseOrderData, { keys: ['Vendor Name', 'Vendor Code', 'City', 'material'], includeScore: true, threshold: 0.4 });
...
const tools = [
  ...
  { name: 'get_sales_orders', description: 'Use this tool ONLY to find/view EXISTING sales orders. Filter by customer, material(s), or status if provided. For multiple materials, include all separated by delimiters. Do NOT use for \"how to\", \"process\", or definition questions.', parameters: { "customer": "(Optional) The customer name to filter by.", "material": "(Optional) The material name(s) or ID(s) to filter by. For multiple materials, include all separated by 'and' or commas (e.g., 'pumps and bearings').", "status": "(Optional) The order status to filter by." } },
  { name: 'get_purchase_orders', description: 'Use this tool ONLY to find/view EXISTING purchase orders or vendor list. Filter by vendor, material(s), status, or quantity if provided. For multiple materials, include all separated by delimiters. Do NOT use for \"how to\", \"process\", or definition questions.', parameters: { "vendor": "(Optional) The vendor name or code to filter by.", "material": "(Optional) The material name(s) or ID(s) to filter by.", "status": "(Optional) The order status to filter by.", "quantity": "(Optional) The exact numeric quantity to filter by." } }
];
...
async function callGroqLLM(systemPrompt, userPrompt, isJsonMode = false) {
  ...
  try {
    ...
    headersObj[['Content', 'Type'].join(String.fromCharCode(45))] = 'application/json';
    ...
    if (decision.type === 'tool_call' && decision.tool_name) {
      ...
      switch (decision.tool_name) {
        case 'get_sap_definition': {
          ...
          console.log(`>>> User asked for process or how to: ${askedForProcess}`);

          const kbSearchResults = knowledgeFuse.search(searchTerm);
          console.log(`>>> Found ${kbSearchResults.length} KB results`);
          
          const topResults = kbSearchResults.filter(result => result.score < 0.6).slice(0, 3);
        ...
        case 'get_sales_orders': {
          ...
          if (parameters.material) {
            console.log(`>>> Filtering SO by material: "${parameters.material}"`);
             
            const materials = extractMultipleItems(parameters.material);
            console.log(`>>> Extracted ${materials.length} material:`, materials);
        ...
        case 'get_purchase_orders': {
          ...
          if (parameters.material) {
            console.log(`>>> Filtering PO by material: "${parameters.material}"`);
          ...
          if (parameters.status) {
            ...
            const statusFuse = new Fuse(purchaseOrdersResults, { keys: ['status', 'Status'], includeScore: true, threshold: 0.4 });
            const statusSearchResults = statusFuse.search(parameters.status);
            purchaseOrdersResults = statusSearchResults.map(result => result.item);
          }

          if (parameters.quantity) {
            console.log(`>>> Filtering PO by quantity: "${parameters.quantity}"`);
            const targetQty = String(parameters.quantity).trim();
            purchaseOrdersResults = purchaseOrdersResults.filter(order => {
              const orderQty = String(order.quantity || order.Quantity || order['Quantity'] || 1).trim();
          ...
          toolResult = {
            type: 'table',
            tableColumns: ['Vendor Code', 'Vendor Name', 'City', 'Quantity', 'GST No.', 'Order Value'],
